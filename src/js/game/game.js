...
function Game(gameDomID) {
    ...
    // 添加到 id 为 gameDomID 的元素中
    var gameDiv = document.getElementById(gameDomID);
    ...
    this.updateScoreCallback = function () { };
...
Object.assign(Game.prototype, {
    ...
    createCube: function () {
        ...
        var geometry = cubeType === 'cube' ?
            new THREE.BoxGeometry(this.config.cubeX, this.config.cubeY, this.config.cubeZ) :
    ...
    createModel: function(position) {
        ...
        mtlLoader.load(`res/obj/${name}.mtl`, async function (materials) {
            materials.preload();
    ...
    _updateScore: function (digit) {
        ...
        // 调用回掉函数
        this.updateScoreCallback(digit, this.score);
    },
    ...
    * 返回值 -3：失败，落点在下一个块内边缘
    ...
    getCurrentDistance: function () {
        ...
        var fromType = fromObj.geometry instanceof THREE.BoxGeometry ? 'cube' : 'cylinder';
        var toObj = this.cubes[this.cubes.length - 1];
        var toPosition = toObj.position;
        var toType = toObj.geometry instanceof THREE.BoxGeometry ? 'cube' : 'cylinder';
